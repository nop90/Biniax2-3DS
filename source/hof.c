...
#define csHOFName		"sdmc:/3ds/Biniax2/hof.bnx2"	/* File name */
...
BNX_BOOL hofInit()
{
	...
	{
		strcpy( Hof.arcade[ i ].name, "Empty                        " );
		Hof.arcade[ i ].score = 0; //(cHofEntries - i) * cHofInitScore;

		strcpy( Hof.tactic[ i ].name, "Empty                        " );
		Hof.tactic[ i ].score = 0;//(cHofEntries - i) * cHofInitScore;
	}
...
{
	BNX_INT32		startTime;
	BNX_INT16		viewoption = cGfxHofKeyboardUp; //cGfxHofNoKeyboard;
	BNX_INT16		curPos = 0;
	BNX_INT16		i, j;
	char			cChar = 0;

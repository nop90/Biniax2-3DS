...
//		flags = SDL_SWSURFACE|SDL_ANYFORMAT;
	...
	_Gfx.screen2 = SDL_SetVideoMode( 320, cGfxScreenY, cGfxColorDepth, flags );
	if ( _Gfx.screen == 0 )
	{
		return BNX_FALSE;
	}
...
//		SDL_SetAlpha( _Gfx.logo, SDL_SRCALPHA | SDL_RLEACCEL, (Uint8) alpha );
		...
		SDL_FillRect( _Gfx.screen, NULL, SDL_MapRGBA(0 , 0, 0, 0, 255-alpha) );
...
/*		switch ( hofview )
		{
			case cGfxHofKeyboardUp:
				...
				SDL_BlitSurface( _Gfx.keyboard, NULL, _Gfx.screen2, &pos );
				break;
...
void gfxMessageBox( BNX_INT16 x, BNX_INT16 y, const char *text )
{
	...
	xt = x + ( (BNX_INT16) _Gfx.window->w >> 1 ) - ( ( (BNX_INT16) strlen( text ) * (int)(cGfxFontSizeX*scaleY) ) >> 1 );
	yt = y + ( (BNX_INT16) _Gfx.window->h >> 1 ) - ( (int)(cGfxFontSizeY*scaleY) >> 1 );
...
BNX_BOOL gfxLoadResources()
{
	...
	bLoad &= gfxLoadImage( "romfs:/graphics/background0.png", &_Gfx.background[ cModeRealtime ]);
	bLoad &= gfxLoadImage( "romfs:/graphics/background1.png", &_Gfx.background[ cModeTurn ]);
	bLoad &= gfxLoadImage( "romfs:/graphics/background2.png", &_Gfx.background[ cModeMultiplayer ] );

	bLoad &= gfxLoadImage( "romfs:/graphics/window.png", &_Gfx.window );
	bLoad &= gfxLoadImage( "romfs:/graphics/splash.png", &_Gfx.splash );
	bLoad &= gfxLoadImage( "romfs:/graphics/logo.png", &_Gfx.logo );
	bLoad &= gfxLoadImage( "romfs:/graphics/help.png", &_Gfx.help );
	bLoad &= gfxLoadImage( "romfs:/graphics/font.png", &_Gfx.font );
	bLoad &= gfxLoadImage( "romfs:/graphics/roadmap.png", &_Gfx.roadmap );

	// Loading Game Elements
	bLoad &= gfxLoadImage( "romfs:/graphics/element0.png", &_Gfx.elements[ 0 ] );
	bLoad &= gfxLoadImage( "romfs:/graphics/element1.png", &_Gfx.elements[ 1 ] );
	bLoad &= gfxLoadImage( "romfs:/graphics/element2.png", &_Gfx.elements[ 2 ] );
	bLoad &= gfxLoadImage( "romfs:/graphics/element3.png", &_Gfx.elements[ 3 ] );
	bLoad &= gfxLoadImage( "romfs:/graphics/elementB.png", &_Gfx.elements[ 4 ] );

	bLoad &= gfxLoadImage( "romfs:/graphics/movecount.png", &_Gfx.movecount );
	bLoad &= gfxLoadImage( "romfs:/graphics/keyboard.png", &_Gfx.keyboard );
	bLoad &= gfxLoadImage( "romfs:/graphics/cursor0.png", &_Gfx.cursors[ 0 ] );
	bLoad &= gfxLoadImage( "romfs:/graphics/cursor1.png", &_Gfx.cursors[ 1 ] );
	bLoad &= gfxLoadImage( "romfs:/graphics/marker0.png", &_Gfx.markers[ 0 ] );
	bLoad &= gfxLoadImage( "romfs:/graphics/marker1.png", &_Gfx.markers[ 1 ] );

	bLoad &= gfxLoadImage( "romfs:/graphics/particle0.png", &_Gfx.part[ 0 ] );
	bLoad &= gfxLoadImage( "romfs:/graphics/particle1.png", &_Gfx.part[ 1 ] );
	bLoad &= gfxLoadImage( "romfs:/graphics/particle2.png", &_Gfx.part[ 2 ] );

	// Set alpha
//	SDL_SetAlpha( _Gfx.window, SDL_SRCALPHA | SDL_RLEACCEL, cGfxWinBlending );

	// Load Help Text
	gfxLoadHelp( "romfs:/text/help.txt" );
...
void gfxGetVirtualChar( BNX_GAME *game, BNX_INP *inp )
{
	...
	if ( inp->mousePress == BNX_TRUE && cfgGetTouch() == BNX_TRUE )
	{
		...
		{
			case cModeTurn:
				ary = (((inp->mouseY - cGfxKeyboardY2*scaleY) * cGfxKeyGridY) / _Gfx.keyboard->h);
				break;
			case cModeRealtime:
				ary = (((inp->mouseY - cGfxKeyboardY1*scaleY) * cGfxKeyGridY) / _Gfx.keyboard->h);
				break;
			default:
				return;
		}
